...
class Crossover(object):
    ...
    def _choose_one_parent(self):  # 二元锦标赛选择 binary tournament selection
                    ...
                    if offspring_len > 1:
                        ...
                        for i in range(1, offspring_len):
                            if offspring[i] != begin:
                                status = True
                                break
                        if status and r2 < 0.5:  # exchange交换两个基因位
                            ...
                            offspring[pos2] = tmp
                        elif status:  # insert一个位置插入到另一位置
                            pos1, pos2 = random.randint(0, offspring_len - 1), random.randint(0, offspring_len - 1)
                            status = True
                            while status:  # 保证两个位置之前的元素不能全部相同（否则相当于失效）
                                if pos1 == pos2:
                        ...
                        mutation_type = random.randint(1, 2)
                if mutation_type == 1:
                    if offspring_len > 1:  # alter变换某位基因
                        pos = random.randint(0, offspring_len - 1)
                        tmp = offspring[pos]
                        while offspring[pos] == tmp:
                            offspring[pos] = random.randint(0, 3)
                    else:
                        mutation_type = 2
                if mutation_type == 2:  # add添加基因
                    pos = random.randint(0, offspring_len)  # 下标来说加了1
