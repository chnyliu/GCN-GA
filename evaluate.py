...
class FitnessEvaluate(object):
    def __init__(self, individual, device, Log, sampler):
        self.indi = individual  # 个体，包含基因型（01）、表型（网络）
        self.device = device
        self.log = Log
    ...
    def evaluate(self, args, gen_no, indi_no, fitness_records, best_fitness, best_genotype, best_lr, best_wd,
        ...
        try:
            ...
            for epoch in range(args.total_epochs):
                train_adj, train_fea = self.sampler.stub_sampler()
                train_adj, train_fea = train_adj.to(self.device), train_fea.to(self.device)
                val_adj, val_fea = self.sampler.get_val_set()
                val_adj, val_fea = val_adj.to(self.device), val_fea.to(self.device)

                model.train()
                optimizer.zero_grad()
                out = model(train_fea, train_adj)
                loss_train = functional.nll_loss(out[idx_train], labels[idx_train])  # 计算训练集损失
                loss_train.backward()  # 损失后向传播
                optimizer.step()  # 优化参数
                loss_train = loss_train.item()

                model.eval()
                output = model(val_fea, val_adj)
                loss_val = functional.nll_loss(output[idx_val], labels[idx_val])  # 验证集损失
                loss_val = loss_val.item()
                # predict = output[idx_val].max(1)[1].type_as(labels[idx_val])
                # correct = predict.eq(labels[idx_val]).double()
                # acc_val = correct.sum() / len(labels[idx_val])
                ...
                if loss_val < fitness:
                    ...
                    if fitness < best_fitness:
                        ...
                        status = True
                        model.eval()
                        test_adj, test_fea = self.sampler.get_test_set()
